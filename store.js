...
// ── Automation: Cek Berita Baru Pemko Medan ───────────────
export const getBeritaPemkoAutoConfig = () => {
  const d = readJSON('berita_pemko_auto.json');
  return {
    enabled:            !!d.enabled,
    modePing:           !!d.modePing,
    modeBroadcast:      !!d.modeBroadcast,
    pingNumbers:        Array.isArray(d.pingNumbers) ? d.pingNumbers : [],
    channelJid:         (d.channelJid || '').trim(),
    intervalMinutes:    Math.max(5, parseInt(d.intervalMinutes || 30, 10)),
    lastCheckedAt:      d.lastCheckedAt      || null,
    lastSeenUrl:        d.lastSeenUrl        || null,
    lastNewArticleUrl:  d.lastNewArticleUrl  || null,
    lastNewArticleTitle:d.lastNewArticleTitle|| null,
    lastNewArticleAt:   d.lastNewArticleAt   || null,
  };
};

export const setBeritaPemkoAutoConfig = (cfg) => {
  const prev = readJSON('berita_pemko_auto.json');
  writeJSON('berita_pemko_auto.json', {
    ...prev,
    enabled:         Boolean(cfg.enabled),
    modePing:        Boolean(cfg.modePing),
    modeBroadcast:   Boolean(cfg.modeBroadcast),
    pingNumbers:     Array.isArray(cfg.pingNumbers)
                       ? cfg.pingNumbers.map(n => n.trim()).filter(Boolean)
                       : (prev.pingNumbers || []),
    channelJid:      (cfg.channelJid || '').trim(),
    intervalMinutes: Math.max(5, parseInt(cfg.intervalMinutes || 30, 10)),
  });
};

export const updateBeritaPemkoAutoState = (patch) => {
  const prev = readJSON('berita_pemko_auto.json');
  writeJSON('berita_pemko_auto.json', { ...prev, ...patch });
};
